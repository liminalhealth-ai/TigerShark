...
class Claim(Facade, X12LoopBridge):
    class _ServiceInfo(X12LoopBridge):
        ...
        prior_authorization_number = ElementAccess(
            "REF", 2, qualifier=(1, "G1"))
        predetermination_of_benefits_id = ElementAccess(
            "REF", 2, qualifier=(1, "G3"))
        ...
        allowed_amount = ElementAccess(
            "AMT", 2, qualifier=(1, "B6"), x12type=Money)
        per_day_limit = ElementAccess(
            "AMT", 2, qualifier=(1, "DY"), x12type=Money)
        deduction_amount = ElementAccess(
            "AMT", 2, qualifier=(1, "KH"), x12type=Money)
        ...
        def get_actual_deductible(self):
            return (self.claim_adjustments
                    .patient_responsibility.total_amount('1'))

        def get_actual_coinsurance(self):
            return (self.claim_adjustments
                    .patient_responsibility.total_amount('2'))

        def get_actual_copayment(self):
            return (self.claim_adjustments
                    .patient_responsibility.total_amount('3'))
    ...
    class _ClaimAdjustments(X12LoopBridge):
        def __init__(self, aLoop, *args, **kwargs):
            super(Claim._ClaimAdjustments, self).__init__(
                aLoop, *args, **kwargs)
            self.contractual_obligation = ClaimAdjustment(
                aLoop, qualifier=(1, "CO"))
            self.correction_and_reversal = ClaimAdjustment(
                aLoop, qualifier=(1, "CR"))
            self.other = ClaimAdjustment(aLoop, qualifier=(1, "OA"))
            self.payor_initiated_reductions = ClaimAdjustment(
                aLoop, qualifier=(1, "PI"))
            self.patient_responsibility = ClaimAdjustment(
                aLoop, qualifier=(1, "PR"))
    ...
    repriced_claim_reference_number = ElementAccess(
        "REF", 2, qualifier=(1, "9A"))
    adjusted_repriced_claim_reference_number = ElementAccess(
        "REF", 2, qualifier=(1, "9B"))
    ...
    original_reference_number = ElementAccess("REF", 2, qualifier=(1, "F8"))
    prior_authorization_number = ElementAccess("REF", 2, qualifier=(1, "G1"))
    predetermination_of_benefits_number = ElementAccess(
        "REF", 2, qualifier=(1, "G3"))
    insurance_policy_number = ElementAccess("REF", 2, qualifier=(1, "IG"))
    ssn = ElementAccess("REF", 2, qualifier=(1, "SY"))

    provider_commercial_number = ElementAccess("REF", 2, qualifier=(1, "G2"))

    # Dates
    date_expiration = ElementAccess("DTM", 2, qualifier=(1, "036"), x12type=D8)
    date_received = ElementAccess("DTM", 2, qualifier=(1, "050"), x12type=D8)
    date_statement_period_start = ElementAccess(
        "DTM", 2, qualifier=(1, "232"), x12type=D8)
    date_statement_period_end = ElementAccess(
        "DTM", 2, qualifier=(1, "233"), x12type=D8)

    def __init__(self, anX12Message, *args, **kwargs):
        super(Claim, self).__init__(anX12Message, *args, **kwargs)
        ...
        self.corrected_insured = NamedEntity(anX12Message, qualifier=(1, "74"))
        self.service_provider = NamedEntity(anX12Message, qualifier=(1, "82"))
        self.crossover_carrier = NamedEntity(anX12Message, qualifier=(1, "TT"))
        self.corrected_priority_payer = NamedEntity(
            anX12Message, qualifier=(1, "PR"))
